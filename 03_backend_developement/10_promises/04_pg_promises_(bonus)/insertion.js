...
function insertionData(result,data,cat_id){
  pool.connect((err, client,done) => {
    ...
    for (let i=0;i<result.length;i++) {
      ...
      case "category_products":
        client.query("INSERT INTO category_products (id_product,id_category) VALUES ($1::uuid,$2::uuid);",
          [result[i].id,cat_id], (err, res) => {
            if (err) {
              // console.log(err.stack);
            }
          });
